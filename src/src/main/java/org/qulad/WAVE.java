...
public class WAVE {
    ...
    private WAVE(String path) {
        ...
        decoded = new AudioFormat(encoding, sampleRate, audioInputStream.getFormat().getSampleSizeInBits(), numChannels, bytesPerFrame, frameRate, false);
    ...
    */
            ...
            while (audioInputStreamDecoded.read(rawData) != -1) {
                ...
                for (int i = 0; i < totalFrameLength; i++) {
                    ...
                    if (numChannels == DUAL_CHANNEL) {
                        ...
                        // Since little endian, interpret array pos as least sig byte
    ...
    public XYChart.Series<Double, Double> calculateSeries(int[] channel) {
        ...
        int max_value = Arrays.stream(channel).max().orElse(Integer.MIN_VALUE);
        int min_value = Arrays.stream(channel).min().orElse(Integer.MAX_VALUE);
    ...
    public XYChart.Series<Double, Double> calculateSeries(double[] channel) {
        ...
        double max_value = Arrays.stream(channel).max().orElse(Double.NaN);
        double min_value = Arrays.stream(channel).min().orElse(Double.NaN);
